...
export default function useHandleVocabularyRemove() {
  ...
  const handleVocabularyRemove = useCallback(
    async (vocabulary: VocabularyType | null) => {
      ...
      try {
        const audioSrcs = extractMultipleAudioAndImageSrc(
          vocabulary.audio_name
        );
        const imgSource1 = extractMultipleAudioAndImageSrc(
          vocabulary.front_desc_html
        );
        const imgSource2 = extractMultipleAudioAndImageSrc(
          vocabulary.back_desc_html
        );

        const combinedSources = [
          ...new Set([...audioSrcs, ...imgSource1, ...imgSource2]),
        ];
        const ifSourceIsUnique: string[] = [];

        await Promise.all(
          combinedSources.map(async (element) => {
            const response = await window.electronAPI.invoke(
              "check-if-img-or-audio-exists",
              {
                vocabularyId: vocabulary.vocabulary_id,
                html: `%${element}%`,
              }
            );
            if (response[0].count === 0 && element) {
        ...
        console.error(
          "Error during vocabulary deletion or file removal:",
          error
        );
      }
    },
